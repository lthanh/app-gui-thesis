...
public class Respond_NewsFeed extends Packet {
    ...
    public Respond_NewsFeed(int isScroll, String userIDReq, String listPost) {
        super(Packet.RES_NewsFeed, (17 + listPost.length()));

        contents[index + 0] = (byte) isScroll;
        ...
        for (m = 0; m < userIDReq.length(); m++) {
            contents[(index + 1 + m)] = tempuserIDPost[m];
        }
        ...
        for (i = 0; i < listPost.length(); i++) {
            contents[(index + 17 + i)] = tempListPost[i];
//            System.out.println("userName : [" + i + "]" + contents[(index + 22 + i)]);
    ...
    public String getUserIDReq() {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            temp = temp + (char) (contents[index + 1 + i]);
        }
//        System.out.println("POST -userID: " + userID);
    ...
    public String getListPost() {
        String listPost = "";
        for (int i = (index + 17); i < (contents.length); i++) {
