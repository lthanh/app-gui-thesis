...
public class checkUserOnlineAction {
    ...
    public void checkUserOnline() {
        ...
        System.out.println("\n checkUserOnlineAction: LIST PONG userPongObject: " + userPongObject.getIdUserLogin());
        System.out.println("\n checkUserOnlineAction: LIST PONG userPongObject: " + userPongObject.getUserName());
        System.out.println("\n checkUserOnlineAction: LIST PONG userPongObject: " + userPongObject.getStatus());

        System.out.println("\n checkUserOnlineAction: INTO checkListUserInPong Status");
    ...
    public Vector<Friends> checkFriendOnline(Vector<Friends> userInPong, Vector<Friends> friendList) {
        for (int i = 0; i < userInPong.size(); i++) {
            for (int j = 0; j < friendList.size(); j++) {
                ...
                System.out.println("checkUserOnlineAction GO INSIDE 2 FOR LOOP checkFriendOnline ");

                if (friendList.get(j).getIdUserLogin().equals(userInPong.get(i).getIdUserLogin())) {
                    System.out.println("checkUserOnlineAction setStatus - ONLINE ");
                    friendList.get(j).setStatus(Preferences.ONLINE);
                    System.out.println("checkUserOnlineAction setStatus - ONLINE " + friendList.get(j).getStatus());
                } else {
                    if (friendList.get(j).getStatus().equals(Preferences.ONLINE)) {
                        ...
                        System.out.println("checkUserOnlineAction AFTER  - getCountOffline : " + friendList.get(j).getCountOffline());

                    }

                    if (friendList.get(j).getCountOffline() == 3) {
                        System.out.println("checkUserOnlineAction setStatus - getCountOffline = 3: " + friendList.get(j).getCountOffline());
                        friendList.get(j).setStatus(Preferences.OFFLINE);
                        System.out.println("checkUserOnlineAction AFTER  getCountOffline = 3: " + friendList.get(j).getStatus());
                    }
                }
            }
        }
